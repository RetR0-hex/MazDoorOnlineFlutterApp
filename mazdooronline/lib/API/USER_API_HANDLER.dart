...
class UserApiHandler extends APIHandler {
  ...
  Future<void> register_labor(String fullName, String phoneNumber, String email,
      String password, int category_id) async {
    try {
      Response response;
  ...
  Future<void> register_user(String fullName, String phoneNumber, String email,
  ...
  Future<LaborData> get_labor_data_by_id(int id) async {
    LaborData laborData = LaborData();
    try {
      Response response;
      response = await dio.get('/api/v1/users/get-user-by-id/${id.toString()}',
          options: Options(headers: JWT_TOKEN_DATA().getAccessHeader()));
      if (response.statusCode != 200) {
        throw Exception("Error in communicating with server");
      }

      laborData.update_values_from_json(response.data['user']);
      return laborData;
    } catch (e) {
      if (e is DioError) {
