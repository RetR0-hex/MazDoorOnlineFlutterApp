...
class OrderApiHandler extends APIHandler{
  Future<bool> create_new_order(int category_id, String desc) async{
    try {
      ...
      // order.add_order(response.data["order"]["id"], response.data["order"]["category_id"],
      //                 response.data["order"]["creator_id"], response.data["order"]["desc"], response.data["order"]["category"]["name"],);
  ...
  Future<OrderDetail> get_order_detail_by_id(int order_id) async {
    try {
      Response response = await dio.get("/api/v1/orders/get-order-by-id/order/$order_id",
          options: JWT_ACCESS_TOKEN_OPTIONS);

      Response response2 = await dio.get("/api/v1/orders/get-location-by-id/order/$order_id",
          options: JWT_ACCESS_TOKEN_OPTIONS);
